...
class HideDirective {
  ...
  initDisplayStyle() {
    if (this._displayStyle === undefined) {
      const /** @type {?} */ displayStyle = this._elementRef.nativeElement.style
          .display;
...
class DataTableColumnDirective {
  ...
  getCellColor(row, index) {
    if (this.cellColors !== undefined) {
      return this
        /** @type {?} */ .cellColors(row.item, row, this, index);
    }
  }
  /**
   * @return {?}
    ...
    if (this.dataTable.rowClick.observers.length > 0) {
      this._listeners.push(
        this.renderer.listen(this.elementRef.nativeElement, "click", event =>
          this.dataTable.rowClicked(this, event)
        )
      );
    }
    if (this.dataTable.rowDoubleClick.observers.length > 0) {
      this._listeners.push(
        this.renderer.listen(this.elementRef.nativeElement, "dblclick", event =>
          this.dataTable.rowDoubleClicked(this, event)
        )
      );
    }
  }
...
DataTableRowComponent.ctorParameters = () => [
  {
    ...
    decorators: [{ type: Inject, args: [forwardRef(() => DataTableComponent)] }]
  },
  { type: Renderer2 },
  { type: ElementRef }
];
...
class DataTableComponent {
  ...
  sortColumn(column) {
    if (column.sortable) {
      const /** @type {?} */ ascending =
          this.sortBy === column.property ? !this.sortAsc : true;
  ...
  _isResizeInLimit(columnElement, dx) {
    ...
    if (
      ...
      (dx >= 0 &&
        /** @type {?} */ (columnElement.nextElementSibling).offsetWidth + dx <=
          this.resizeLimit)
    ) {
      return false;
    }
  ...
  _notify() {
    ...
    if (!loading) {
      if (this.pagination) {
        this.paginationNotification = this.labels.paginationText
          .replace(
            "{from}",
            "" +
              (Math.ceil(this.itemCount / this.limit) !== 0
                ? this.offset + 1
                : "0")
          )
          .replace(
      ...
      if (this.columns !== undefined && this.sortBy !== undefined) {
        const /** @type {?} */ col = /** @type {?} */ (this.columns
            .toArray()
            .find(column => column.property === this.sortBy));
...
class DataTableHeaderComponent {
  ...
  onChange(event) {
    ...
    const /** @type {?} */ columnName = event /** @type {?} */.target.parentElement.textContent
        .trim();
    const /** @type {?} */ interpolateParams = {
        column_name: columnName,
        title: this.dataTable.title
      };
    this.dataTable.columnSelectorNotification = (isChecked
...
DataTableHeaderComponent.propDecorators = {
  onClickHandler: [
    { type: HostListener, args: ["document:click", ["$event"]] }
  ],
...
class DataTablePaginationComponent {
  ...
  validate(event) {
    ...
    if (newValue !== this.page) {
      this.page =
        event.target.value > this.maxPage
          ? this.maxPage
          : newValue < 1
          ? 1
          : newValue;
...
export {
  ...
  DataTableColumnDirective as DataTableColumn,
  DataTableRowComponent as DataTableRow,
  DataTablePaginationComponent as DataTablePagination,
  DataTableHeaderComponent as DataTableHeader,
  DataTableResource,
  HideDirective as ɵb,
  MinPipe as ɵc,
  PixelConverter as ɵa
};
